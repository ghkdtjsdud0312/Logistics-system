...
| 📅 스케줄 | - | 배차 일정(출발~도착 시각)과 입고 예정을 월간 달력으로 표시, 항목을 누르면 해당 화면으로 이동 |
| ⚙️ 기준정보 | 상품관리 / 창고·위치관리 / 차량·기사관리 | 상품, 창고→구역→위치 트리, 차량·기사 |

> 창고·위치관리에는 **3D 보기** 탭(Three.js)이 있어 위치 칸을 재고 상태 색(재고 없음/가용/예약/소진)으로 보여 주고, 상태가 바뀌면 SSE로 갱신됩니다. 나중에 추가: 지도. 배송현황·배송완료·스케줄·입고/반품 상세 화면은 상태가 바뀌면 SSE로 자동 갱신됩니다.
...
- 지도와 프런트 자동 테스트는 구현하지 않았습니다. 성능 테스트(k6)는 대시보드 요약 캐시 OFF/ON 비교만 수행했습니다(`k6/README.md`, 노트북 단일 환경·1,000건 기준).
- 스케줄 달력은 조회 API의 전체 목록을 화면에서 월별로 거르며 일정 편집은 없습니다.
