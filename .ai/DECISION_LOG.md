...
## ADR-022 스케줄(달력), 상세 화면, 화면 자동 갱신
- 날짜: 2026-09-26 / 상태: Accepted
- 결정: 사용자 요청으로 (1) 스케줄 메뉴를 추가해 배차 일정과 입고 예정을 월간 달력으로 보여 준다(라이브러리 없이 직접 구현, 새 API 없이 기존 조회 API 사용), (2) 입고·반품 상세 화면을 추가하고, (3) 배송현황·배송완료·스케줄·상세 화면이 SSE 이벤트로 자동 갱신되게 한다(이벤트를 300ms 모아 한 번만 재조회).
- 한계: 달력은 조회 API가 반환하는 전체 목록을 프런트에서 월별로 거른다(데모 규모 가정). 일정 편집·드래그는 없다. 지도는 여전히 범위 밖이다.
