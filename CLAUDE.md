...
5. **엄격한 파일 크기 제약 및 모듈화 (Max Code Length & Reusability)**:
   ...
   - ⚠️ **[초과 시 절대 규칙]** 만약 비즈니스 로직의 특성상(예: 복잡한 상태 전이·재고 계산 등) 한 파일이 300~400자를 넘길 수밖에 없는 상황이 발생하면, **코드를 작성하기 전에 반드시 사용자에게 상황을 설명하고, 어떻게 분리할 것인지 계획을 보고하여 명시적인 승인을 얻은 후에만 진행**하십시오.
...
> **현재 상태**: 위 11개 문서는 이미 `.ai/`에 생성되어 있고, 미결정 항목은 `.ai/DECISION_LOG.md`의 "미결정 항목" 절에 기록되어 있습니다. 도메인 코드도 이미 일부(입고/출고/배차/배송) 구현이 시작된 상태이며, 2026-09-25 방향 전환(ADR-011~018)으로 일부는 재정의·폐기 예정이므로, 새로 합류하는 에이전트는 이 절의 "코드 작성 금지"를 문자 그대로 적용하지 말고 `.ai/TASKS.md`의 진행 상태를 먼저 확인하십시오.
...
- `ai/logistics-ops-ai-harness/README.md`: 문서 읽기 순서와 3일 개발 계획
- `.ai/`: 설계 문서 11종 + `SCOPE.md`, `DEMO_SCENARIO.md`
- `docks/development/DAY_01~03`: 일차별 개발 지시서 (3일 계획)
